...
/**
 * Factory for basket/address JSON API client
 *
 * @package Client
 * @subpackage JsonApi
 */
...
{
	/**
	 * Creates a basket/address client object.
	 *
	...
	public static function createClient( \Aimeos\MShop\Context\Item\Iface $context, $path, $name = null )
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'client/jsonapi/basket/address/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Client\\JsonApi\\Basket\\Address\\' . $name : '<not a string>';
