...
/**
 * Factory for subscription JSON API client
 *
 * @package Client
 * @subpackage JsonApi
 */
...
{
	/**
	 * Creates a subscription client object.
	 *
	 * @param \Aimeos\MShop\Context\Item\Iface $context Shop context instance with necessary objects
	 * @param string $path Name of the client separated by slashes, e.g "subscription"
	...
	public static function createClient( \Aimeos\MShop\Context\Item\Iface $context, $path, $name = null )
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'client/jsonapi/subscription/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? '\\Aimeos\\Client\\JsonApi\\Subscription\\' . $name : '<not a string>';
		...
		$classname = '\\Aimeos\\Client\\JsonApi\\Subscription\\' . $name;
